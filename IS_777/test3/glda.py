...
class LDA:
    ...
    def inference(self):
        ...
        for m, doc in enumerate(self.docs):
            z_n = self.z_m_n[m]
            n_m_z = self.n_m_z[m]
            alpha_n_m_z = self.get_alpha_n_m_z(m)
            for n, t in enumerate(doc):
                ...
                p_z = self.n_z_w[:, t] * alpha_n_m_z / self.n_z
    ...
    def word_dist_with_voca(self, voca, topg = None):
        '''
        ...
        for g in range(self.G):
            for w in np.argsort(-phi[g])[:topg]:
                result[g][voca[w]] = phi[g, w]
        return result

    def output_word_dist_with_voca(self, voca, topg = 10):
        word_dist = self.word_dist_with_voca(voca, topg)
        for k in word_dist:
            word_dist[k] = sorted(word_dist[k].items(),
                key=lambda x: x[1], reverse=True)
            for w, v in word_dist[k]:
                self.log(self.logger.debug, "Group", [k, w, v])
